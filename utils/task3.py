...
parser.add_argument('--result_dir', type=str, default='results/ques3_')
...
result_dir = 'results/ques3_' + time_str
def mkdir(path):
...
def return_marker_results(source, s, var, types):
    ...
    source_.to_csv(result_dir+'/ques_3_source_marker_class_{}_.csv'.format(types))
    ctc.to_csv(result_dir+'/ques_3_ctc_marker_class_{}_.csv'.format(types))

def return_marker_results_ques3(source, s, var, types):
    adata = sc.AnnData(source, obs=s,var=var)

    source_ =  return_marker_result(adata, 'ctc_selected_'+str(types))
    source_.to_csv(result_dir+'/ques_3_ctc_marker_class_{}_new.csv'.format(types))
def return_type_samples_ques3(types, target_arr, target_labels):
    arr_select, arr_last, label_select, label_last= sampling_by_class(target_arr, target_labels, types)
    return arr_select, arr_last
...
print('target_arr, target_labels', target_arr.shape, target_labels.shape)
for i in label_for_inves:
    arr_select, arr_last = return_type_samples_ques3(i, target_arr, target_labels)
    s_ = labels_generate(i, arr_select, arr_last, l=['ctc_selected_', 'ctc_last_'])
    total = np.append(arr_select, arr_last, axis=0)
    return_marker_results_ques3(total, s_, var, i)