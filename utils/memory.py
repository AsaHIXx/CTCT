...
class Memory(nn.Module):
    ...
    def update_center_by_simi(self, batch_center, flags):
        ...
        self.memory = update_wei * self.memory + (1 - update_wei) * batch_center
        self.memory = F.normalize(self.memory, p=2, dim=-1)
        self.memo_change = self.calc_memo_change(self.memory,
                                                 old_center)  # torch.matmul(self.memory, old_center.transpose(0, 1)).mean()
    ...
    def forward(self, feat, label, t=1.0, onehot=False):
        ...
        simis = simis / t
        if onehot:
            log_prob = torch.nn.functional.log_softmax(simis, dim=1)
            loss = -torch.sum(log_prob * label) / label.size(0)
        else:
